...
public class BlobAgent extends Agent<MyAMAS, MyEnvironment>{
	...
	private Integer[] determinerPositionVoisins() {
		...
		// Nous pouvons d�finir 2 droites (consid�rons notre blob (x;y) et le voisin (X;Y) ):
		...
		for(BlobAgent voisin : voisins)
		{
			...
			if(coo[1] > coo[0] + ordonnee1)
				if(coo[1] > -coo[0] + ordonnee2)
					res[0]++; //appartient � la zone Nord
				else
					res[3]++; // appartient � la zone West
			else
				if(coo[1] < -coo[0] + ordonnee2)
					res[2]++;  // appartient � la zone Sud
				else
					res[1]++;	//appartient � la zone Est			
		}
		return res;
	}
