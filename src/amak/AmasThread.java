...
/**
 * Thread servant d'interface entre AMAK et les autres packages (application ou positions)
 * <ul>
 * <li>Il est initialisé par le controller de l'IHM (application.Controller) </li>
...
public class AmasThread extends Thread{
	/** Controller (=ihm) permettant de dialoguer avec lui.
	 * Concerne à la fois l'affichage des blobs et la mise à jour des curseurs
	 */
	Controller controller;
	/** MyAMAS de AMAK. Permet d'acceder principalement à l'environnement pour atteindre les blobs */
	MyAMAS myAmas;
	/** ServerThread permettant de dialoguer avec le thread Serveur. 
	 * Récupère surtout les nouvelles positions et renvoie les infos des Blobs
	 */ 
	 ...
	 * @param nbBlobs
	 * 		nombre de Blobs réels à creer dans TO. Fourni par l'utilisateur dans l'IHM.
	 */
	 ...
	 * <li> Si une mauvaise coordonnée est demandée (en dehors de la map) : 
	 *      la demande est ignorée. </li>
	 * <li> Cette méthode est appelée soit par application.Controller si en mode Test,
	 * soit par position.ServerThread ou position.ServerThreadAcceleration si en mode experience. </li>
	 * </ul>
	 * 
	 * @param b 
	...
	/**
	 * Demande d'adoption d'un Blob : on permet donc le passage d'un agent mûr de TO vers Tr
	 * et on retourne le migrant en question (pour que le thread appelant ait les infos sur ce blob).
	 * 
	 * <p>Méthode appelée par le thread ConnectedClient.</p>
	 * <p> Procédé : <ul>
	 * <li> On demande à l'environnement pour récupérer un blob mûr </li>
	 * <li> Renvoie null si aucun blob n'est mûr. </li>
	 * <li> On demande à cet agent d'effectuer son passage de To à Tr par la méthode t0_to_tr.
	 * Le changement de terrain sera en réalité effectif qu'à la fin du cycle de AMAK. </li>
	 * </ul>
	 * @see t0_to_tr
	 * @param coo
	 * 			coordonnée où doit se placer le blob à sa sortie sur Tr
	 * @return
	 * 			l'agent (de type Migrant) qui s'est fait adopter.
	 */
	...
	/**
	 * demande de passage de l'agent blob de To vers Tr avec une coordonnée précise dans Tr
	 * <p> Cette méthode fait une vérification sur la coordonnée demandée. 
	 * Si une mauvaise coordonnée est fournie, 
	 * on effectue tout de même le passage dans Tr mais avec une coordonnéee aléatoire valide. </p>
	 * <p> appel à la fonction {@link amak.Migrant#t0_to_tr() Migrant.t0_to_tr} avec la coordonnée valide. </p>
	 * @param blob
	...
	public void t0_to_tr(Migrant blob, double[] coo){
		if(!myAmas.getEnvironment().isValideInTi(coo)) 
			// Les coordonnées fournies ne sont pas valides. Je lui affecte une valeur alÃ©atoire dans la salle de diametre
	...
	/**
	 * demande de passage d'un agent (Migrant) de To à Tr, sans coordonnée prédéfinie.
	 * l'agent sera donc placé à une coordonnée aléatoire dans Tr.
	 * <p> Cette méthode fait appel à la méthode {@link amak.Migrant#t0_to_tr() Migrant.t0_to_tr}. 
	 * Ce changement sera effectif qu'à la fin du cycle de AMAS. </p>
	 * @param blob
	 ...
	 * <p> Cette méthode fait appel à la méthode {@link amak.Migrant#t0_to_tr()} </p>
	 * <p> Le changement ne sera effectif qu'à la fin du cycle de AMAK </p>
	 * <p> L'agent sera placé à une coordonnée aléatoire dans To </p>
	 * 
	 * @param blob
	...
	/**
	 * démarre le thread, et initialise MyAMAS de AMAK, avec un nombre de blob qui a été défini dans l'IHM.
	 * <p> Cette méthode met à jour la liste des blobs hibernants dans To de la classe application.Controller (l'IHM) </p>
	 */
	public void run(){
	 ...
	 * <ul><li> utilisé notamment pour récupérer des infos sur les blobs par les autres threads </li></ul>
