...
public class Blob {
	...
	// on cr�e un blob � la position (xcor, ycor) de couleur et de forme al�at	public Blob(double xcor, double ycor, boolean reel)
	{
		coordonnee = new double[2];
		coordonnee[0] = xcor;
		coordonnee[1] = ycor;
	...
	/*
	// fonction qui � partir de coordonn�es initiales, propose de nouvelles coordonn�es � un certain rayon (l	public double[] nouvellesCoordonnees(){
		double[] res = new double[2];
		// coo[0] - pas < res[0] < coo[0] + pas
		res[0] = (Math.random() * 2 * pas) - pas + coordonnee[0];
		
		// j'utilise l'equation d'un cercle de rayon pas.
		// (res[0] - coo[0])� + (res[1] - coo[1])� = p		// � partir de res[0], j'ai 2 solutions possible pour res[1]. 1 positive, une n�gative. choisissons al�atoire		double sign = 1;
		if (Math.random() < 0.5)
			sign = -1;
	...
	// genere des coordonn�es cart�siennes aleatoires dans un cercle de diametre D et de centre D/2;		public double[] genererCoordonneeAleaDansCercle(double D){
			boolean isOk = false;
			double[] res = new double[2];
			double xcor = 0;
			double ycor = 0;
	...
	private void centrerBlob(ArrayList<double[]> res) {
		...
		// � chaque position, j'ajoute le vecteur (ancien centre (50,50) - nouveau Centre )
